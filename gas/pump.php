<?
	...
	if(isset($_POST['BREAK'])) {
		...
		mysql_query("update pumps set Status='notok' where PumpNumber='1'") or die("Error: ". mysql_error());
	} else {
	$result = mysql_query("select AmountAuthorized, Halted, Reset, Status from pumps where PumpNumber='1'", $db) 
	...
	if($row['Halted'] == "false" && $row['Reset'] == "false" && $row['Status'] == "ok") {
		...
		if(isset($_SESSION['var2']) && $Authorized <> 0 && !isset($_POST['gallons'])) {
			...
			$result1 = mysql_query("select AmountAuthorized from pumps where PumpNumber='1'");
			...
			$result = mysql_query("select Reset from pumps where PumpNumber='1'") 
				or die("Error: ".mysql_error());
			...
			mysql_query("update items set Quantity='".($row[0] - $amount_to_pump_this_second)."' where ItemName='".$_SESSION['type']."'", $db) 
				or die("Error: ".mysql_error());
			mysql_query("update pumps set AmountAuthorized='".$_SESSION['gallons']."' WHERE PumpNumber='1'", $db) or die("Error: ".mysql_error());
			if(($row[0] > 250) && ($row[0] - $_SESSION['gallons'] < 250)) {
				alert($_SESSION['type']);
			}
			echo '<META http-equiv="REFRESH" content="1;url='.$_SERVER['PHP_SELF'].'">';
			if($_SESSION['gallons'] < .01) {
				mysql_query("update pumps set InUse='no' where PumpNumber='1'", $db) or die("Error: ".mysql_error());
		...
		echo '<META http-equiv="REFRESH" content="1;url='.$_SERVER['PHP_SELF'].'">';
		echo '<b>PUMP 1 BROKEN</b>';
		die();
	}
	else if($row['Halted'] == "true") {
		echo '<META http-equiv="REFRESH" content="1;url='.$_SERVER['PHP_SELF'].'">';
		echo '<b>PUMP 1 DISABLED</b>';
		die();
	}
	else {
		echo '<META http-equiv="REFRESH" content="1;url='.$_SERVER['PHP_SELF'].'">';
		echo '<b>PUMP 1 RESET</b>';
		session_destroy();
