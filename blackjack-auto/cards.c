...
int main( int argc, char * argv[] )
{
    ...
    while(1) 
    {
        ...
        while (!game_done)
        {
            ...
            output_results = num_player_cards = num_dealer_cards = 0;
            ...
            else
            {
                ...
                if (CARD(dealer[0]) == "A")
                {
                    ...
                    writer(buff, newSd);
                    sprintf(buff, "Do you want to purchase insurance for $%d? (y/n):\n", bet / 2);
                    writer(buff, newSd);
                    reader(buff, newSd);
                    if (buff[0] == 'y')
                    {
...
void matchOutput(int total_dealer_cards, int total_player_cards, int blackjack, int insurance, int bet, int newSd, int *cash)
{
    ...
    else if (total_dealer_cards == 21 && total_player_cards != 21 && !insurance)
    {
        if (blackjack)
        {
            sprintf(buff, "Dealer blackjack!! - You lose $%d\n", bet);
            writer(buff, newSd);
        }
        else
        {
...
int placeBet( int cash, int newSd )
{
    ...
    else
    {
        ...
        sprintf(buff, "\nPlace bet ($%d available, 0 to quit):\n", cash);
        writer(buff, newSd);
